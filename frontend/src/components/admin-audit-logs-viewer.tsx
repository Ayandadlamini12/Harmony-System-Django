...
export function AdminAuditLogsViewer() {
  ...
  // Fetch access logs
  const fetchLogs = async (pageNum: number, filterType: string, append = false) => {
    try {
      setLoading(true);
      let url = `/api/audit-logs/?page=${pageNum}`;
      if (filterType) {
  ...
  useEffect(() => {
    setPage(1);
    fetchLogs(1, entityTypeFilter, false);
  }, [entityTypeFilter]);

  const handleLoadMore = () => {
    const nextPage = page + 1;
    setPage(nextPage);
    fetchLogs(nextPage, entityTypeFilter, true);
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 p-4 border border-[var(--hh-border)] bg-white/70 backdrop-blur-md rounded-2xl shadow-xs">
        <div className="flex items-center gap-2.5">
          <SlidersHorizontal size={16} className="text-slate-500" />
          <span className="text-sm font-bold text-slate-700">Filter Activities:</span>
          
          <select
            value={entityTypeFilter}
            onChange={(e) => setEntityTypeFilter(e.target.value)}
            className="text-xs font-bold text-slate-700 bg-slate-50 hover:bg-slate-100 border border-[var(--hh-border)] rounded-lg px-3 py-2 cursor-pointer transition-all focus:outline-none focus:ring-1 focus:ring-[var(--hh-purple)]"
          >
            <option value="">All System Activities</option>
            <option value="patient">Patient Profiles & Registrations</option>
            <option value="patientprofile">Medical Clinical Histories</option>
            <option value="vital">Vitals Measurements</option>
            <option value="case">Clinical Cases</option>
            <option value="visit">Visit Encounters</option>
            <option value="patientdocument">Documents & Agreements</option>
            <option value="patientjourney">Queue Transitions</option>
            <option value="appointment">Appointments Scheduled</option>
          </select>
        </div>

        <div className="flex items-center justify-end gap-2">
          <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400 bg-slate-100 border border-slate-200/50 rounded-md px-2.5 py-1">
          ...
          <Button 
            ...
            disabled={loading}
            className="text-xs font-bold text-slate-600 hover:text-[var(--hh-purple)] border-slate-200 active:scale-[0.98] gap-1.5 flex items-center"
          >
      ...
      {/* Main Audit List Container */}
      <div className="bg-white rounded-2xl border border-[var(--hh-border)] overflow-hidden shadow-xs">
        <div className="px-6 py-4 border-b border-[var(--hh-border)] bg-slate-50/50 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 text-[10px] font-bold text-emerald-600 uppercase tracking-widest">
            <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse mr-1"></span> Secured
          </div>
        </div>
        ...
        <div className="p-0">
          {logs.length === 0 && loading && page === 1 ? (
            <div className="flex flex-col items-center justify-center py-20 text-slate-400 gap-3">
              <Loader2 size={40} className="animate-spin text-[var(--hh-purple)]" />
              <span className="text-xs font-bold uppercase tracking-wider">Syncing cryptographic audit logs...</span>
            </div>
          ) : logs.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-slate-400">
              ...
              <p className="text-2xs text-slate-400 mt-1">Adjust filters or register new records to populate the system log.</p>
            </div>
          ) : (
            <div className="divide-y divide-slate-100">
              {logs.map((entry) => {
                ...
                // Filter out technical database columns
                ...
                return (
                  <div key={entry.id} className="transition-colors duration-150 hover:bg-slate-50/10">
                    <div 
                      className={`flex flex-col md:flex-row md:items-center justify-between p-5 gap-3 cursor-pointer select-none ${isExpanded ? 'bg-slate-50/40' : ''}`}
                      onClick={() => toggleExpand(entry.id)}
                    >
                      <div className="flex items-start gap-4">
                        <div className="mt-1 shrink-0 bg-slate-100/70 p-1.5 rounded-lg border border-slate-200/50">
                          {isExpanded ? (
                        ...
                        <div className="grid gap-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <span className="text-[10px] font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-md border border-slate-200/40">
                      ...
                      <div className="flex items-center justify-end md:pl-0 pl-10">
                        {hasChanges ? (
                          <span className="text-3xs font-extrabold text-amber-600 bg-amber-50 border border-amber-200 rounded-md px-2 py-0.5 flex items-center gap-1 uppercase tracking-wider">
                            <FileText size={10} />
                            {filteredChangedFields.length} fields modified
                          </span>
                        ) : (
                    ...
                    {/* Expanded Section */}
                    {isExpanded && (
                      <div className="bg-slate-50/50 p-5 pl-14 border-t border-slate-100/50 space-y-4 animate-slide-down">
                        <div className="text-xs text-slate-600 font-medium leading-relaxed">
                          <strong className="font-bold text-slate-800">Activity Detail:</strong> {entry.details || defaultDesc}
                        </div>

                        {entry.user_agent && (
                          <div className="text-[10px] font-semibold text-slate-400 tracking-wide font-mono break-all bg-white p-3 rounded-lg border border-slate-200/40 shadow-2xs leading-relaxed">
                            ...
                            <table className="min-w-full divide-y divide-slate-100 text-xs text-left">
                              <thead>
                                <tr className="bg-slate-50/80 font-extrabold text-slate-500 uppercase tracking-widest text-[9px] border-b border-slate-100">
                                  ...
                                  <th className="px-4 py-2.5 bg-red-50/30 text-red-700">Before Change</th>
                                  <th className="px-4 py-2.5 bg-emerald-50/30 text-emerald-700">After Change</th>
                                </tr>
                              </thead>
                              <tbody className="divide-y divide-slate-100 bg-white">
                                {filteredChangedFields.map(([field, values]) => (
                                  <tr key={field} className="hover:bg-slate-50/20 transition-colors">
                                    ...
                                    <td className="px-4 py-2 bg-red-50/10 text-red-600 line-through font-medium text-[11px]">
                                      {values.before !== null && values.before !== undefined ? String(values.before) : "—"}
                                    </td>
                                    <td className="px-4 py-2 bg-emerald-50/10 text-emerald-800 font-bold text-[11px]">
      ...
      {hasMore && (
        <div className="pt-4 text-center">
          <Button 
            ...
            className="text-xs font-bold uppercase tracking-wider px-8 py-4 border-slate-200 text-slate-600 hover:text-[var(--hh-purple)] active:scale-[0.98]"
          >
            {loading ? (
              <>
