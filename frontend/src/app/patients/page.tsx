...
export default async function PatientsPage({ searchParams }: { searchParams: Promise<{ search?: string }> }) {
  ...
  return (
    <AppShell title="Patient directory" action={<Link className="hh-button" href="/patients/new">Register patient</Link>}>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              ...
              {patients.results.length === 0 && (
                <tr>
                  <td className="px-5 py-10 text-center text-[#66736d]" colSpan={6}>No patients found.</td>
                </tr>
              )}
            </tbody>
          </table>
